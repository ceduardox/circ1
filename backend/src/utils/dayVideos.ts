...
// Videos de las tareas diarias que apuntan a los archivos locales servidos por el frontend.
// Idempotente: solo actualiza el day_content si la URL actual no coincide con la esperada.
const dayVideos = [
...
export async function syncDayVideos(): Promise<number> {
  let updated = 0;
  for (const v of dayVideos) {
    ...
    if (content.url === v.url && content.provider === v.provider) continue;
    await prisma.dayContent.update({
      where: { id: video.id },
      data: { content: { ...content, url: v.url, provider: v.provider, duration: v.duration, author: v.author, description: v.description } },
    });
    updated++;
  }
  return updated;
